...
#
    ssh -o StrictHostKeyChecking=no ${compute_shard[$n]}  "git clone https://github.com/google/cityhash && cd cityhash/ && ./configure && make all check CXXFLAGS='-g -O3' && sudo make install && cd .. && sudo add-apt-repository ppa:ubuntu-toolchain-r/test && sudo apt-get update && sudo apt install -y gcc-10 g++-10 && sudo apt-get install -y libnuma-dev numactl htop libmemcached-dev memcached libboost-all-dev libgflags-dev" & # python3.6
